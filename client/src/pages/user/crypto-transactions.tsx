...
export default function CryptoTransactions() {
  ...
  const { data: cryptoPrices, refetch: refetchPrices, isRefetching: isRefetchingPrices } = useQuery({
    ...
    refetchInterval: activeWallets.length > 0 ? 60000 : false, // Update every 1 minute only if there are active wallets
  ...
  return (
    <DashboardLayout title="تراکنش ارز دیجیتال">
      <div className="space-y-6">
        ...
        <Tabs 
          defaultValue={
            (walletData?.walletAddress && activeWallets.includes('tron')) ? "tron" : 
            (walletData?.usdtWalletAddress && activeWallets.includes('usdt')) ? "usdt" : 
            (walletData?.rippleWalletAddress && activeWallets.includes('ripple')) ? "ripple" : 
            (walletData?.cardanoWalletAddress && activeWallets.includes('cardano')) ? "cardano" : 
            "tron"
          } 
          className="w-full"
        >
          <TabsList className={`grid w-full ${
            activeWallets.length === 4 ? 'grid-cols-4' :
            activeWallets.length === 3 ? 'grid-cols-3' :
            activeWallets.length === 2 ? 'grid-cols-2' :
            'grid-cols-1'
          }`}>
            {walletData?.walletAddress && activeWallets.includes('tron') && (
              <TabsTrigger value="tron">
                ترون (TRX)
              </TabsTrigger>
            )}
            {walletData?.usdtWalletAddress && activeWallets.includes('usdt') && (
              <TabsTrigger value="usdt">
                تتر (USDT)
              </TabsTrigger>
            )}
            {walletData?.rippleWalletAddress && activeWallets.includes('ripple') && (
              <TabsTrigger value="ripple">
                ریپل (XRP)
              </TabsTrigger>
            )}
            {walletData?.cardanoWalletAddress && activeWallets.includes('cardano') && (
